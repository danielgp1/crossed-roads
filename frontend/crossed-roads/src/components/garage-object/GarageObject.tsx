...
const GarageObject: React.FC<GarageObjectProps> = ({ id, first_name, last_name, profile_name, current_color, profile_pic_url, longitude, latitude }: GarageObjectProps) => {
    ...
    return areFriends ? (
        <div className='garage-object'>
            ...
            {isNavigationOpen && <MapsNavigation setIsNavigationOpen={setIsNavigationOpen} first_name={first_name} last_name={last_name} profile_pic_url={profile_pic_url} longitude={longitude} latitude={latitude}/>}
        </div>
    ) : null;
}
